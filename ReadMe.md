...
- `/discover`：拍品列表，支持排序、状态和类别筛选，筛选条件同步到 URL 查询参数；拍品卡片使用固定高度，缩略图贴合卡片完整高度并保留四角圆角，拍品状态以黑色半透明徽标嵌入缩略图中下部；列表不展示开拍时间和右侧动作按钮，点击卡片主体进入商品详情，蓝色 `商家>` 是唯一独立入口。
...
- `/discover` 拍品卡固定为统一高度，左侧缩略图贴合卡片完整高度并保留四角圆角；拍品状态改为黑底白字的半透明徽标，嵌入缩略图中下部；卡片不再显示开拍时间和 `去看看 / 已结束 / 去出价` 等动作按钮，点击卡片主体进入商品详情，蓝色 `商家>` 保留为独立商家入口。
