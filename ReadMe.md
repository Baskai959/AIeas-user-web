...
- 实时排行榜榜首交接按 ghost 动画终帧完成，同步切换目标行内容；目标第一名行不再叠加 enter-up 动画，ghost 横条复用榜首样式并在终帧后一帧清理，避免榜首在收尾阶段出现半透明闪烁或字体跳变
...
- 快速出价弹层中位于出价选择器和立即出价按钮之间的提示收敛为少量高优先级信息：本地冷却倒计时、实时连接未就绪、手慢价变、竞拍队列拥挤、通用出价未通过，以及后端未命中前端映射的拒绝 `payload.message`。其他本地校验或终态原因仍会禁用提交或改变按钮/倒计时状态，但不再在该位置额外展示提示。
...
- 直播间竞拍清单的开拍时间 badge 增加宽度测量：在移动端窄宽度下若会被截断，则隐藏该时间信息，只保留拍品状态。
