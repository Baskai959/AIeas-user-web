...
- `/discover`：拍品列表，支持排序、状态和类别筛选，筛选条件同步到 URL 查询参数；拍品卡片使用固定高度，左侧拍品缩略图为 1:1 正方形并以居中 `cover` 裁剪展示，拍品状态以黑色半透明徽标嵌入缩略图中下部；列表不展示开拍时间和右侧动作按钮，点击卡片主体进入商品详情，蓝色 `商家>` 是唯一独立入口。
...
- 全局竞拍氛围提醒层：基于现有实时事件派生 `领先 / 被超越 / 竞拍延时 / 竞拍结束 / 竞拍成功` 弹窗动画，并承载最后 10 秒倒计时提醒。提醒动画同一时间只显示一个，新的竞拍提醒会立即中断并替换正在播放的提醒；倒计时提醒优先级低于其他竞拍提醒，当其他竞拍提醒正在显示时不会同时出现。提醒层固定覆盖在直播间所有 UI 上方，包含快速出价、拍品详情和竞拍清单，但设置为不接收触控事件，不影响出价、关闭弹层或评论操作；系统评论和快速出价反馈仍保留，弹窗仅承担即时氛围反馈。
...
### 2026-06-09 拍品缩略图与竞拍清单信息对齐

- 直播间竞拍清单中，开拍时间移到拍品状态右侧且仅显示时间值，时间 badge 使用等宽数字并按文本内容收缩边框，状态、时间、标题、简介和价格形成更紧凑的纵向信息层级。
- 系统内拍品缩略图统一为 1:1 正方形容器，真实图片仅使用等比缩放和居中 `cover` 裁剪，不做拉伸；覆盖发现页拍品卡、直播间竞拍清单、订单记录和出价摘要等拍品媒体入口。

### 2026-06-08 发现页拍品卡与订单列表精简

- `/discover` 拍品卡固定为统一高度，左侧缩略图保留四角圆角；拍品状态改为黑底白字的半透明徽标，嵌入缩略图中下部；卡片不再显示开拍时间和 `去看看 / 已结束 / 去出价` 等动作按钮，点击卡片主体进入商品详情，蓝色 `商家>` 保留为独立商家入口。
- `/orders?tab=...` 删除订单列表上方的 compact heading 和 `order-list-page-section` 包装，页签下方直接承载结果列表；订单页容器固定头部和页签，仅下方记录列表响应纵向滑动，且列表行高按内容自适应，少量记录时不会把卡片拉伸到异常高度。
